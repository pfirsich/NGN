...
namespace ngn {
    ...
    Mesh* Renderer::fullScreenMesh = nullptr;
    VertexShader* Renderer::deferredVertexShader = nullptr;

    const int Renderer::FORWARD_AMBIENT_PASS = 1;
    const int Renderer::FORWARD_LIGHT_PASS = 2;
    const int Renderer::SHADOWMAP_PASS = 4;
    const int Renderer::GBUFFER_PASS = 8;
    const int Renderer::DEFERRED_LIGHT_PASS = 16;
    const int Renderer::DEFERRED_AMBIENT_PASS = 32;
    ...
    void Renderer::staticInitialize() {
        ...
        Shader::globalShaderPreamble += "#define NGN_PASS_FORWARD_AMBIENT " + std::to_string(FORWARD_AMBIENT_PASS) + "\n";
        Shader::globalShaderPreamble += "#define NGN_PASS_FORWARD_LIGHT " + std::to_string(FORWARD_LIGHT_PASS) + "\n";
        Shader::globalShaderPreamble += "#define NGN_PASS_FORWARD(x) (x >= " + std::to_string(FORWARD_AMBIENT_PASS) + " && x <= " + std::to_string(FORWARD_LIGHT_PASS) + ")\n\n";
        Shader::globalShaderPreamble += "#define NGN_PASS_SHADOWMAP_PASS " + std::to_string(SHADOWMAP_PASS) + "\n";
        Shader::globalShaderPreamble += "#define NGN_PASS_DEFERRED_GBUFFER " + std::to_string(GBUFFER_PASS) + "\n";
        Shader::globalShaderPreamble += "#define NGN_PASS_DEFERRED_AMBIENT " + std::to_string(DEFERRED_AMBIENT_PASS) + "\n";
        Shader::globalShaderPreamble += "#define NGN_PASS_DEFERRED_LIGHT " + std::to_string(DEFERRED_LIGHT_PASS) + "\n";
        Shader::globalShaderPreamble += "#define NGN_PASS_DEFERRED(x) (x >= " + std::to_string(GBUFFER_PASS) + " && x <= " + std::to_string(DEFERRED_LIGHT_PASS) + ")\n\n";
...
uniform ngn_LightParameters ngn_light;
        ...
        // PostEffect has these too. Can they share?
        // TODO: delete these
        deferredVertexShader = new VertexShader;
        deferredVertexShader->setSource(R"(
...
void main() {
    ...
    Renderer::Renderer() : mDeferredAmbientShaderProgram(nullptr), mDeferredLightShaderProgram(nullptr),
        ...
        glm::ivec2 size(Window::currentWindow->getSize());
        // Normals (X (10 Bit), Y (10)), free (10), material id (2)
        mGBufferTextures[0].setStorage(PixelFormat::RGB10_A2, size.x, size.y);
        mGBuffer.attachTexture(Rendertarget::Attachment::COLOR0, mGBufferTextures[0]);
        // Diffuse Albedo R(8), G(8), B(8), Specular Exponent (8)
        mGBufferTextures[1].setStorage(PixelFormat::RGBA, size.x, size.y);
        mGBuffer.attachTexture(Rendertarget::Attachment::COLOR1, mGBufferTextures[1]);
        // Depth + Stencil
        mGBufferTextures[2].setStorage(PixelFormat::DEPTH24, size.x, size.y);
        mGBuffer.attachTexture(Rendertarget::Attachment::DEPTH, mGBufferTextures[2]);

        mDeferredFragmentShader = ResourceHandle<FragmentShader>(Resource::getPrepare<FragmentShader>("media/shaders/ngn/blinnPhong.frag"));
    }

    void Renderer::updateShaders() {
        if(mDeferredFragmentShader.dirty()) {
            FragmentShader* frag = mDeferredFragmentShader.getResource();

            if(mDeferredAmbientShaderProgram) delete mDeferredAmbientShaderProgram;
            mDeferredAmbientShaderProgram = new ShaderProgram;
            if(!mDeferredAmbientShaderProgram->compileAndLinkFromStrings(
                    frag->getFullString("#define NGN_PASS NGN_PASS_DEFERRED_AMBIENT\n").c_str(),
                    deferredVertexShader->getFullString().c_str())) {
                delete mDeferredAmbientShaderProgram;
                mDeferredAmbientShaderProgram = nullptr;
            }

            if(mDeferredLightShaderProgram) delete mDeferredLightShaderProgram;
            mDeferredLightShaderProgram = new ShaderProgram;
            if(!mDeferredLightShaderProgram->compileAndLinkFromStrings(
                    frag->getFullString("#define NGN_PASS NGN_PASS_DEFERRED_LIGHT\n").c_str(),
                    deferredVertexShader->getFullString().c_str())) {
                delete mDeferredLightShaderProgram;
                mDeferredLightShaderProgram = nullptr;
            }
        }
    }
    ...
    void Renderer::render(SceneNode& root, Camera& camera) {
                        ...
                        for(size_t i = 0; i < linearizedSceneGraph.size(); ++i) {
                            ...
                            if(mesh) {
                                ...
                                if(mat->getStateBlock().getDepthWrite()) {
                                    ...
                                    if(!pass) pass = mat->getPass(FORWARD_AMBIENT_PASS);
        ...
        mGBuffer.bind();
        if(autoClear) clear();
        for(auto node : linearizedSceneGraph) {
            Mesh* mesh = node->getMesh();
            if(mesh) {
                Material* mat = node->getMaterial();
                assert(mat != nullptr);
                Material::Pass* pass = mat->getPass(GBUFFER_PASS);

                if(pass && pass->getShaderProgram()) {
                    if(!pass->getStateBlock().getBlendEnabled()) {
                        renderQueue.emplace_back(mat, pass, mesh);
                        RenderQueueEntry& entry = renderQueue.back();
        ...
        if(mDeferredAmbientShaderProgram) {
            mDeferredAmbientShaderProgram->bind();
            mDeferredAmbientShaderProgram->setUniform("ngn_gBuffer_normal", mGBufferTextures[0]);
            mDeferredAmbientShaderProgram->setUniform("ngn_gBuffer_material", mGBufferTextures[1]);
            mDeferredAmbientShaderProgram->setUniform("ngn_gBuffer_depth", mGBufferTextures[3]);
            fullScreenMesh->draw();
        }
        ...
        //             Material* mat = node->getMaterial();
        //             assert(mat != nullptr);
        //             Material::Pass* pass = mat->getPass(FORWARD_AMBIENT_PASS);
        ...
        //             Material* mat = node->getMaterial();
        //             assert(mat != nullptr);
        //             Material::Pass* pass = mat->getPass(FORWARD_LIGHT_PASS);
        //             if(pass && pass->getShaderProgram()) {
        //                 if(drawTransparent == pass->getStateBlock().getBlendEnabled()) {
        //                     for(size_t ltype = 0; ltype < LIGHT_TYPE_COUNT; ++ltype) {
